...
def men_page(request):
    ...
    return render(request, 'store/Men.html', {'watches':watches, 'username':name})

def women_page(request):
    Get_name(request);
    watches = Watches_Women.objects.all().order_by('?')
    return render(request, 'store/Women.html', {'watches':watches, 'username':name})
...
def collections_page(request):
    ...
    omegas = watches[0:4]
    zeniths = watches[4:]
    return render(request, 'store/Collections.html', {'omegas':omegas, 'zeniths':zeniths, 'username':name })

def cart(request):
    Get_name(request);
    return render(request, 'store/Cart.html', {'username':name})
